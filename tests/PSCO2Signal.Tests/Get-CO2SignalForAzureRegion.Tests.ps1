Describe "Get-CO2SignalForAzureRegion.Tests" {
    BeforeAll {
        $module = Import-Module -Name $PSScriptRoot/../../src/PSCO2Signal -PassThru -Force
        $module | Should -Not -BeNullOrEmpty
        $env:SIGNALTOKEN | Should -Not -BeNullOrEmpty
    }

    Context " Get-CO2SignalForAzureRegion with named parameters" {
        BeforeAll {
            $testCase = Get-CO2SignalForAzureRegion -Region "westeurope" -AuthToken $env:SIGNALTOKEN
        }
    ...
    Context " Get-CO2SignalForAzureRegion with pipeline parameters" {
        BeforeAll {
            $testCase = "westeurope" | Get-CO2SignalForAzureRegion -AuthToken $env:SIGNALTOKEN
        }
